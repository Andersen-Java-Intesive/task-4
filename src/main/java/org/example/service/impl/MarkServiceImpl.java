...
public class MarkServiceImpl implements MarkService {
    ...
    @Override
    public LinkedHashSet<Pair<Date, Map<User, Double>>> groupMarksByLessonDate(LinkedHashSet<Mark> allMarks) {
        Map<Date, Map<User, Double>> groupedByDate = new HashMap<>();
        ...
        LinkedHashSet<Pair<Date, Map<User, Double>>> result = new LinkedHashSet<>();
        for (Map.Entry<Date, Map<User, Double>> entry : groupedByDate.entrySet()) {
            result.add(new Pair<>(entry.getKey(), entry.getValue()));
        }
        return result;
    }
    @Override
    public Map<User, Double> getUserTotalScores(LinkedHashSet<Pair<Date, Map<User, Double>>> lessons) {
        LinkedHashSet<User> users = userRepository.getAll();
        Map<User, Double> userTotalScores = new HashMap<>();
        for (User user : users) {
            userTotalScores.put(user, 0.0);
        }
        for (Pair<Date, Map<User, Double>> lesson : lessons) {
